...
from CTR_Prediction.DeepFM.DeepFM import DeepFM


def _load_data():
    dfTrain = pd.read_csv(config.TRAIN_FILE, sep='\t', low_memory=False)
    dfTest = pd.read_csv(config.TEST_FILE, sep='\t', low_memory=False)
    dfTrain[config.CATEGORICAL_COLS] = dfTrain[config.CATEGORICAL_COLS].fillna('-1', )
    dfTest[config.CATEGORICAL_COLS] = dfTest[config.CATEGORICAL_COLS].fillna('-1', )

    dfTrain[config.NUMERIC_COLS] = dfTrain[config.NUMERIC_COLS].fillna(0, )
    dfTest[config.NUMERIC_COLS] = dfTest[config.NUMERIC_COLS].fillna(0, )

    cols = [c for c in dfTrain.columns if c not in ["if_click"]]
    cols = [c for c in cols if (not c in config.IGNORE_COLS)]

    X_train = dfTrain[cols].values
    y_train = dfTrain["action_detailPageShow"].values
    X_test = dfTest[cols].values
    pids_test = dfTest["id"].values
    uids_test = dfTest["userId"].values
